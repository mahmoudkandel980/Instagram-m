...
const UploadStoryImage = (props: ModeInterFace) => {
    const { mode } = props;
    ...
    const { updateStoriesHandler } = updateTargetCtx;

    const hideModelHandler = () => {
        showStoryModelHandler(false, "");
    };
    ...
    const submitHandler = async () => {
        ...
        let userStoryData: StoryInterface[] = [];
        ...
        try {
            ...
            // user has story
            // @ts-ignore
            userStoryData.push({ ...postSnap.data() });
        } catch (error) {
            // user hasn't story
            userStoryData.length = 0;
        }

        if (userStoryData[0]?.userName) {
            // if document exist in firebase server
            const storyRef = doc(db, "stories", `${userData.userName}`);
            const imgs = userStoryData[0].imgs;
            ...
            await updateDoc(storyRef, {
                // @ts-ignore
                imgs: arrayUnion(...imgs, ...img),
            });
        } else {
            // if document dosen't exist in firebase server
            ...
            await setDoc(
                doc(db, "stories", userData.userName),
                currentUserStory
            );
        }
